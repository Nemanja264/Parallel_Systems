...
int main(int argc, char** argv)
{
	...
	MPI_Bcast(students, 5, student_type, root, MPI_COMM_WORLD);
	for (int i = 0; i < 5; i++)
	{
		cout << "Process " << rank << " received student: " << students[i].index << ", " << students[i].ime 
			<< " " << students[i].prezime << ", average grade: " << students[i].avg_grade << endl;
	}
